...
df_C2_KNZ.cleanest$plot<-as.factor(df_C2_KNZ.cleanest$plot)
...
KNZ_plotlevel_metrics<-df_E1_E2_KNZ_clean_plot_metrics%>%
  select(-1,-2,-5)%>%
  bind_rows(K1_mass,df_C2_KNZ.cleanest,df_A1_A2_KNZ.clean,df_J2_KNZ.clean.richness.evenness,df_I1_I2_KNZ.clean,RaMPs_ANPP)%>%
  select(1:12)
...
KNZ_metadata<-df_E1_E2_metadata%>%
  select(-2,-5)%>%
  bind_rows(K1_meta,df_J2_KNZ.clean.richness.evenness,df_C1_KNZ.cleanest,A1_A2_metadata,df_I1_I2_KNZ.clean,RaMPs_PlantCommunityComposition)%>%
  select(1:18)%>%
  distinct()
