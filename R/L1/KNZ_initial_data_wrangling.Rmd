...
KNZ_specieslevel_abundance<-df_E1_KNZ_species_level%>%
  ...
  select(-10:-17)%>%
  #bind_rows(df_J2_KNZ.clean.richness.evenness)%>% notspecies identity need to confirm before including
  #select(-10:-22)#%>%
...
KNZ_metadata<-df_E1_E2_metadata%>%
  ...
  bind_rows(K1_meta,C1_Meta)#,df_I1_I2_KNZ.clean#df_J2_KNZ.clean.richness.evenness,RaMPs_PlantCommunityComposition)%>%
  select()%>%
  distinct()%>%
