...
date: "2023-12-11" Last Modified: "Oct 6, 2024"
...
RaMPs_ANPP <- RaMPs_ANPP_working %>% 
  ...
  mutate(site = "KNZ",
         ramps = "RampNo",
         experiment = "RaMPs",
         treatment  = case_when(RampNo %in% ambient_ramps ~ "control",#made the ambient the control since the actual control were not needed
                                RampNo %in% altered_ramps ~ "altered",
                                RampNo %in% control_ramps ~ "ambient"),
...
RaMPs_PlantCommunityComposition <- ramps %>% 
  ...
  mutate(relative_abundance = plot_biomass_spp/plot_biomass) %>% 
  dplyr::rename(year = RecYear,
                subplot = plot) %>% 
  mutate(site = "KNZ",
         ramps = "RampNo",
         experiment = "RaMPs",
         treatment  = case_when(RampNo %in% ambient_ramps ~ "control",#made ambient control 
                                RampNo %in% altered_ramps ~ "altered",
                                RampNo %in% control_ramps ~ "ambient"),
         block = case_when(RampNo %in% c(1:5) ~ "Block1",
                           RampNo %in% c(6:10) ~ "Block2",
...
#we are excluding this dataset-just leaving the code in here
```{r}
...
KNZ_specieslevel_abundance<-df_E1_KNZ_species_level%>%
  select(DataCode,RecType,PlotID,)
  bind_rows(K1_spp,df_C1_KNZ.cleanest)#,df_I1_KNZ.clean,df_J2_KNZ.clean.richness.evenness)%>%
  select(1:9)%>%
...
KNZ_plotlevel_metrics<-df_E1_E2_KNZ_clean_plot_metrics%>%
  select(-1,-2,-5)%>%
  bind_rows(K1_mass,df_C2_KNZ_cleanest,df_J2_KNZ.clean.richness.evenness,df_I1_I2_KNZ.clean,RaMPs_ANPP)%>%
  select(1:12)%>%
...
KNZ_metadata<-df_E1_E2_metadata%>%
  select(-2,-5)%>%
  bind_rows(K1_meta,df_J2_KNZ.clean.richness.evenness,C1_Meta,df_I1_I2_KNZ.clean,RaMPs_PlantCommunityComposition)%>%
  select(1:18)%>%
  distinct()%>%
