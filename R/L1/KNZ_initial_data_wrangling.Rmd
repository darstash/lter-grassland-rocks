...
df_C1_KNZ.cleanest<-df_C1_KNZ.cleanest%>%
  ...
  mutate(original_measurement_unit="cover_class")
...
df_C2_KNZ.noreps = mutate(df_C2_KNZ.noreps, unique_id = paste(site, DataCode, year, plot_trt, sep="_"))

#Removing unnecessary columns
df_C2_KNZ.noreps = df_C2_KNZ.noreps[-c(2,4,6)]
...
df_C2_KNZ_cleanest <- df_C2_KNZ.noreps %>%
  right_join(C1_diversity, by=c("year_plot_trt"))%>%
  mutate(measurement_scale_biomass="0.1")
df_C2_KNZ_cleanest = df_C2_KNZ_cleanest[-c(3,8)]
...
#treatment:
C1_Meta[c('knz', 'watershed', 'year', 'plizot', 'treatment')] <- str_split_fixed(C1_Meta$unique_id, '_', 5)
#drop those new columns besides treatment
C1_Meta<-C1_Meta %>%
  dplyr::select(-knz, -year, -plizot)
C1_Meta<-C1_Meta %>%
...
RaMPs_PlantCommunityComposition<-RaMPs_PlantCommunityComposition%>%
  mutate(uniqueid=uniqueID,abundance=Pseudo_PercCover)
df_A1_KNZ.clean<-df_A1_KNZ.clean%>%
  mutate(relative_abundance=relative_abundance/100)
RaMPs_PlantCommunityComposition$year=as.integer(RaMPs_PlantCommunityComposition$year)
...
RaMPs_ANPP$year<-as.integer(RaMPs_ANPP$year)
...
C1_Meta$year<-as.numeric(C1_Meta$year)
